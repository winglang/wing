...
class $Root extends $stdlib.std.Resource {
  constructor($scope, $id) {
    ...
    {((cond) => {if (!cond) throw new Error("assertion failed: v.at(0) == 123")})((((a,b) => { try { return require('assert').deepStrictEqual(a,b) === undefined; } catch { return false; } })(((obj, args) => { if (args[0] < 0 || args[0] >= v.length) throw new Error("Index out of bounds"); return obj[args[0]]; })(v, [0]),123)))};
...
new $App({ outdir: $outdir, name: "json_static.test", rootConstruct: $Root, plugins: $plugins, isTestEnvironment: $wing_is_test, entrypointDir: process.env['WING_SOURCE_DIR'], rootId: process.env['WING_ROOT_ID'] }).synth();
