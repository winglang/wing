...
module.exports = function ({ b }) {
  class $Closure1 {
    ...
    async handle() {
      {
        ((cond) => {
          if (!cond) throw new Error("assertion failed: b.list().length == 0");
        })((await b.list()).length === 0);
      }
  ...
  class $Closure2 {
    ...
    async handle() {
      await b.put("hello.txt", "world");
      {
        ((cond) => {
          if (!cond)
            throw new Error('assertion failed: b.get("hello.txt") == "world"');
        })((await b.get("hello.txt")) === "world");
      }
    }
  }
  return $Closure2;
