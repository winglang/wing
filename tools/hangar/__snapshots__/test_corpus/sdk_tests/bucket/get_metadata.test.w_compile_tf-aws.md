# [get_metadata.test.w](../../../../../../examples/tests/sdk_tests/bucket/get_metadata.test.w) | compile | tf-aws
...
module.exports = function({ $b }) {
  class $Closure1 {
    ...
    async handle() {
      ...
      const file1Metadata = (await $b.getMetadata("file1.main.w"));
      ...
      const file2Metadata = (await $b.getMetadata("file2.txt"));
      ...
      const file3Metadata = (await $b.getMetadata("file3.txt"));
      ...
      try {
        (await $b.getMetadata("no-such-file.txt"));
      }
      catch ($error_e) {
    ...
    class $Closure1 extends $stdlib.std.Resource {
      ...
      _registerBind(host, ops) {
        if (ops.includes("handle")) {
          $Closure1._registerBindObject(b, host, ["getMetadata", "put"]);
        }
        super._registerBind(host, ops);
      }
    }
    const b = this.node.root.newAbstract("@winglang/sdk.cloud.Bucket",this,"cloud.Bucket");
    this.node.root.new("@winglang/sdk.std.Test",std.Test,this,"test:getMetadata()",new $Closure1(this,"$Closure1"));
  }
}
const $App = $stdlib.core.App.for(process.env.WING_TARGET);
new $App({ outdir: $outdir, name: "get_metadata.test", rootConstruct: $Root, plugins: $plugins, isTestEnvironment: $wing_is_test, entrypointDir: process.env['WING_SOURCE_DIR'], rootId: process.env['WING_ROOT_ID'] }).synth();
