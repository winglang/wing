...
```js
module.exports = function({ s, math_Util }) {
  class $Closure1 {
    constructor({  }) {
    ...
    async handle()  {
      {((cond) => {if (!cond) throw new Error("assertion failed: math.min(s) == 1")})(((await math_Util.min(s)) === 1))};
      {((cond) => {if (!cond) throw new Error("assertion failed: math.max(s) == 5")})(((await math_Util.max(s)) === 5))};
    }
  }
  return $Closure1;
}
...
class $Root extends $stdlib.std.Resource {
  constructor(scope, id) {
    ...
    const s = Object.freeze(new Set([1, 2, 3, 4, 5]));
    {((cond) => {if (!cond) throw new Error("assertion failed: math.min(s) == 1")})(((math.Util.min(s)) === 1))};
    {((cond) => {if (!cond) throw new Error("assertion failed: math.max(s) == 5")})(((math.Util.max(s)) === 5))};
