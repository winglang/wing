    ...
    async handle() {
      {
        ((cond) => {
          if (!cond)
            throw new Error("assertion failed: http.get(website.url).ok");
        })((await $http_Util.get($website_url)).ok);
      }
      const wingEnv =
        (await $http_Util.get($website_url + "/wing.js")).body ?? "";
      ...
      {
        ((cond) => {
          if (!cond) throw new Error("assertion failed: wingEnv == expected");
        })(
          ((a, b) => {
            try {
