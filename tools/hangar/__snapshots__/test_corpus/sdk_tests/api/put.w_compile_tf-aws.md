...
```js
module.exports = function({ $_id, $api_PUT, $body, $user, std_Json }) {
  const std = {
    Json: std_Json,
  };
  class $Closure1 {
...
```js
module.exports = function({ $_id, $api_url, $body, $http_PUT, $user, http_Util, std_Json }) {
  const http = {
    Util: http_Util,
  };
  const std = {
  ...
  class $Closure2 {
    async handle()  {
      ...
      const fetchResponse = (await http.Util.put(url,{
      "method": $http_PUT,
      "headers": Object.freeze({"content-type":"application/json"}),
      "body": ((args) => { return JSON.stringify(args[0], null, args[1]) })([$body]),}
      ));
  ...
  constructor(scope, id) {
    ...
    class $Closure1 extends $stdlib.std.Resource {
      ...
      static _toInflightType(context) {
        const $_id = context._lift(_id);
        const $api_PUT = context._lift(api_PUT);
        const $body = context._lift(body);
        const $user = context._lift(user);
    ...
    class $Closure2 extends $stdlib.std.Resource {
      ...
      static _toInflightType(context) {
        ...
        const $body = context._lift(body);
        const $http_PUT = context._lift(http_PUT);
        const $user = context._lift(user);
