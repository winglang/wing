...
module.exports = function({ $std_Json }) {
  class $Closure1 {
    ...
    async handle(req) {
      return ({"body": ((args) => { return JSON.stringify(args[0], null, args[1]?.indent) })([({"user": ((obj, key) => { if (obj[key] === undefined) throw new Error(`Map does not contain key: "${key}"`); return obj[key]; })(req.vars, "name")})]),"headers": ({"content-type": "application/json"}),"status": 200});
    }
  }
  return $Closure1;
}
...
module.exports = function({ $std_Json }) {
  class $Closure2 {
    ...
    async handle(req) {
      return ({"body": ((args) => { return JSON.stringify(args[0], null, args[1]?.indent) })([({"user": ((obj, key) => { if (obj[key] === undefined) throw new Error(`Map does not contain key: "${key}"`); return obj[key]; })(req.vars, "name"),"age": ((obj, key) => { if (obj[key] === undefined) throw new Error(`Map does not contain key: "${key}"`); return obj[key]; })(req.vars, "age")})]),"headers": ({"content-type": "application/json"}),"status": 200});
    }
  }
  return $Closure2;
}
