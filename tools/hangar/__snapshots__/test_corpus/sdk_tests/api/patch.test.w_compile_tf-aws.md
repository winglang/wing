...
module.exports = function({ $_id, $body, $cloud_HttpMethod, $std_Json }) {
  class $Closure1 {
    ...
    async handle(req) {
      ...
      return ({"status": 200,"body": ((obj, key) => { if (obj[key] === undefined) throw new Error(`Map does not contain key: "${key}"`); return obj[key]; })(req.vars, "id")});
    }
  }
  return $Closure1;
}
...
new $App({ outdir: $outdir, name: "patch.test", rootConstruct: $Root, plugins: $plugins, isTestEnvironment: $wing_is_test, entrypointDir: process.env['WING_SOURCE_DIR'], rootId: process.env['WING_ROOT_ID'] }).synth();
