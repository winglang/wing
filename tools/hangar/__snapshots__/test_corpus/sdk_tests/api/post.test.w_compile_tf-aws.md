...
module.exports = function({ $body, $cloud_HttpMethod, $std_Json }) {
  class $Closure1 {
    ...
    async handle(req) {
      ...
      {((cond) => {if (!cond) throw new Error("assertion failed: req.headers?.get(\"content-type\") == \"application/json\"")})((((a,b) => { try { return require('assert').deepStrictEqual(a,b) === undefined; } catch { return false; } })(((map, key) => { if (!(key in map)) { throw new Error("no value found for key '" + key + "'"); } return map[key]; })(req.headers, "content-type"),"application/json")))};
