...
import { tmpDir, validWingFiles } from "./paths";
import { runWingCommand, sanitize_json_paths } from "./utils";

describe.each(validWingFiles)("%s", (wingFile) => {
  test.concurrent("wing compile --target tf-aws", async ({ expect }) => {
    const args = ["compile", "--target", "tf-aws"];
    const testDir = path.join(tmpDir, `${path.basename(wingFile)}_cdktf`);
    ...
    await runWingCommand(testDir, path.join(wingFile), args, true);
