...
resource Function {
   ...
   */
  inflight invoke_async(payload: Serializable): Serializable;
}
```
...
resource Topic {
   ...
   */
  subscribe(fn: inflight (event: Serializable) => void, opts: TopicOnPublishProps?): void;
...
The table resource represents a relational database table that can be used to store and query data.
