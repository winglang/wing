...
The concrete implementation will be chosen automatically through a `ClassFactory` (see below) when instantiating the polycon.
...
import { Construct } from "constructs";
import { ClassFactory, CaptureClient } from "polycons";
import { TFBucket } from "./resources/tf-bucket.ts";

export class TerraformAWSFactory extends ClassFactory {
  public resolve(
    qualifier: string,
    scope: Construct,
    id: string,
...
Typically this is done through a base "app" class that handles this, such as `cdktf-aws.App` which makes sure to invoke `ClassFactory.register(this, new TerraformAWSFactory())`.
...
```ts
export class MultiCloudFactory extends ClassFactory {
  constructor() {
