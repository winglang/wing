...
This is implemented on a per-target basis to expose the permissions in a useful way.
For the `sim` target this problem is (currently) much more difficult.
...
```toml
[tf-aws]
client_policy = "AcmeClientPolicy"
```

After this policy is created, a role and associated service account must be created for application pods to use it.
This is only needed for `tf-aws` deployment so it can either but inside a check to the `WING_TARGET` environment variable or in a separate class/file:
