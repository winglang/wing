...
- A real product and 5 “hell yes” people using & saying they are loving this
...
- **Deadlines generate focus** - We know that teams that have all the time in the world don't complete anything, we use deadlines to make sure we stay focused and lean
...
This section describes the market trends and the different offering available today for serverless.
...
We are introducing a 100x better developer experience for serverless services that includes an API gateway, a queue and a bucket where results are stored.
...
In this tutorial we will build an image scraper. It is a cloud application that takes a URL of an HTML page from an endpoint, downloads all the images from this page and stores them inside a bucket.
...
2. Create an image-scraper.w file:
```js
 use cloud
...
```sh
$ wing compile --target local ./image-extractor.w 
```

2. A new file was generated as the result of compile image-extractor.wx
```sh
...
In this section you execute `image-extractor.wx` that will be opened by the Wing Console which is bundled as part of the wing command
