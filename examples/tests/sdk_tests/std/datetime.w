...
let d4 = datetime.fromComponents(year: 2023, month: 6, day: 18, hour: 20, min: 18, sec: 25, ms: 177, tz: -180);

assert(d4.timezone == -180);
...
test "inflight datetime" {
  ...
  let d8 = datetime.fromComponents(year: 2023, month: 6, day: 18, hour: 20, min: 18, sec: 25, ms: 177, tz: -180);

  assert(d7.timestampMs == 1689700705177);
  assert(d7.hours == 17);
  assert(d7.min == 18);
  assert(d7.sec == 25);
  assert(d7.ms == 177);
  ...
  assert(d7.timestamp == d8.timestamp);
  assert(d4.toUtc().hours == (d4.hours + (d4.timezone / 60)));
