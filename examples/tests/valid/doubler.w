...
let fn = new Doubler(inflight fn(m: str) -> str {
  return "Hello ${m}!";
});
...
class Doubler2 {
  ...
  makeFunc(handler: inflight fn(num) -> num) -> cloud.Function {
    return new cloud.Function(inflight fn(x: str) -> str {
...
let f = doubler2.makeFunc(inflight fn(x: num) -> num {
  return x * 2;
});
