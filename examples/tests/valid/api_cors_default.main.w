...
test "GET /users has default cors headers" {
 ...
 testing.Assert.equal(headers.get("access-control-expose-headers"), "");
...
test "OPTIONS /users has default cors headers" {
 ...
 testing.Assert.equal(headers.get("access-control-allow-headers"), "Content-Type,Authorization,X-Requested-With");
 testing.Assert.equal(headers.get("access-control-allow-methods"), "GET,POST,PUT,DELETE,HEAD,OPTIONS");
 testing.Assert.equal(headers.get("access-control-allow-origin"), "*");
