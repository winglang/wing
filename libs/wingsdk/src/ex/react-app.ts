...
export const injectWingEnv = (
  indexFilePath: string,
  wingEnv: Record<string, string>
): string => {
  const content = `// Generated by wing\nwindow.wingEnv = ${JSON.stringify(
    wingEnv
  )};`;
  const root = parse(readFileSync(indexFilePath).toString());
  const head = root.querySelector("head");
  const wingScript = head?.getElementById("wing-script");
  if (wingScript) {
    wingScript.innerHTML = content;
  } else {
    head?.appendChild(parse(`<script id="wing-script">${content}</script>`));
  }
