...
export class Platform implements IPlatform {
  ...
  public registerParameters?(inputRegistrar: ParameterRegistrar) {
    const vpcParameter = new PlatformParameter(inputRegistrar, "VPC", {
      ...
      description:
        "Determine whether to create a VPC or use an existing one (if needed)",
      choices: ["new", "existing"],
    });

    const existingVPCIdParameter = new PlatformParameter(
      inputRegistrar,
      "ExistingVPCID",
      {
    ...
    const existingPrivateSubnetIdsParameter = new PlatformParameter(
      inputRegistrar,
    ...
    const existingPublicSubnetIdsParameter = new PlatformParameter(
      inputRegistrar,
    ...
    vpcParameter.addDependentParameter(
      existingPrivateSubnetIdsParameter,
      "existing"
    );
    vpcParameter.addDependentParameter(
      existingPublicSubnetIdsParameter,
      "existing"
    );
  }
}
