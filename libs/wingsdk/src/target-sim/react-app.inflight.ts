...
import { IReactAppClient, REACT_APP_FQN, injectWingEnv } from "../ex";
...
export class ReactApp implements IReactAppClient, ISimulatorResourceInstance {
  ...
  public async init(): Promise<ReactAppAttributes> {
    ...
    const indexPath = join(`${this.path}`, `/public/index.html`);

    // injecting the wingEnv into react's index file
    writeFileSync(
      indexPath,
      injectWingEnv(indexPath, this.environmentVariables)
    );
