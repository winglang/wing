...
test("default topic behavior", () => {
  ...
  cloud.Topic.newTopic(app, "Topic");
...
test("topic with subscriber function", () => {
  // GIVEN
  const app = new tfaws.App({ outdir: mkdtemp() });
  const topic = cloud.Topic.newTopic(app, "Topic");
...
test("topic name valid", () => {
  // GIVEN
  const app = new tfaws.App({ outdir: mkdtemp() });
  const topic = cloud.Topic.newTopic(app, "The-Spectacular_Topic-01");
  const output = app.synth();
...
test("replace invalid character from queue name", () => {
  // GIVEN
  const app = new tfaws.App({ outdir: mkdtemp() });
  const topic = cloud.Topic.newTopic(app, "The%Spectacular@Topic");
  const output = app.synth();
