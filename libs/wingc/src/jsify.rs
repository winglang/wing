...
impl<'a> JSifier<'a> {
	...
	fn jsify_arg_list(
		...
		let mut code = CodeMaker::with_source(&arg_list.span);
		...
		if !structure_args.is_empty() {
			let mut combined_struct_args = CodeMaker::with_source(&arg_list.span);
			codes!(combined_struct_args, "{ ", structure_args, " }");
			args.push(combined_struct_args);
		}
	...
	fn jsify_elif_statements(
		...
		let mut start_code = CodeMaker::default();
		codes!(
			start_code,
			"const ",
			value,
	...
	fn jsify_class(&self, env: &SymbolEnv, class: &AstClass, ctx: &mut JSifyContext) -> CodeMaker {
		ctx.with_class(class, |ctx| {
			...
			let mut code = new_code!(&class.name.span, "class ", &class.name.name);
			...
			if let Some(parent) = &class.parent {
				...
				if let Some(fqn) = &parent_type.as_class().unwrap().fqn {
					...
					codes!(code, " extends ", self.jsify_user_defined_type(&parent, ctx));
				}
			} else {
				// default base class for preflight classes is `core.Resource`
				codes!(code, " extends ", STDLIB_CORE_RESOURCE);
			};
	...
	fn jsify_class_inflight(&self, class: &AstClass, mut ctx: &mut JSifyContext) -> CodeMaker {
		...
		if let Some(parent) = &class.parent {
			codes!(class_code, " extends ", self.jsify_user_defined_type(&parent, ctx));
		}
	...
	fn jsify_register_bind_method(
		...
		let mut bind_method = CodeMaker::with_source(&class.name.span);
